<script lang="ts">
  ...
  let node = $state<HTMLParagraphElement | undefined>();
  const merged = $derived(mergeProps(ctx.getApi().getDescriptionProps(), rest));
  $effect(() => {
    applyCallbackRef(node ?? null, merged);
  });
</script>

<p bind:this={node} {...omitRef(merged)}>{@render children?.()}</p>
