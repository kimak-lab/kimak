...
- `@kimak/motion-gsap` is optional JS motion. It peers `gsap`, targets anatomy selectors, and must not be imported from `@kimak/core` or any headless adapter. Product sugar (`@kimak/ui-*`) binds Button press motion. Docs demos consume `<Button>` only — do not re-bind GSAP there. Default look stays CSS. Loading spinner stays the Tailwind keyframes.
